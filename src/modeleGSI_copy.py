...
def modele_gsi(Vfreestream, alt, reff, Twall):
    ...
    preshock_state = [Tfreestream, Pfreestream, Vfreestream]
    # print("\n preshockstate, T, P, V", preshock_state)

    options1 = {"ratio": 0.2, "robust": "Yes"}
    ...
    mix.setDiffusionModel(
        Xgas, dx2
    )  # to compute diffusive velocity ~ (Xgas - Xwall)/dx
    mix.solveSurfaceBalance()  # solving surface state
    rhoiwall, Twall = mix.getSurfaceState(
        set_state_with_rhoi_T
    )  # getting surface state (updated rhoiwall, Twall unchanged as not solving energy balance)
    # print("\n rhoiwall", rhoiwall)
    ...
    print("qrad", qrad*1e-6)
    print("qdiff", qdiff*1e-6)
    print("qcond", qcond*1e-6)
    print("qadv", qadv*1e-6)
    ...
    return (
        ...
        mblowtot,
        Numberdensity[0],
    )  # , qdiff, qcond, qadv, Numberdensity[0], wdot
...
# print(
#     "results model: qwtot, mblowtot, qdiff, qcond, qadv, Numberdensity[0], wdot",
#     results,
# )


# print(
#     "with cabaret : Qw = 1238689.8407288808 W/m2 with the same input param M=20, reff = 0.5, alt = 60"
# )
